...
之前渲染出来的模型只有头部，我误以为是模型数据没有完全读出来，以为还有一部分顶点信息包含在NodeAttributeType为Skeleton的节点中。今天才发现是因为在初始化zBUffer时，我是用的初始值是std::numeric_limits<float>::min(),但是这个值代表的是正数中float的最小值，表示的是float的最小精度值，而不是我要的float能表示的最小的值。因此只有深度值大于std::numeric_limits<float>::min()的部分才会被渲染出来。应该使用-std::numeric_limits<float>::max()才对。反思了一下，std::numeric_limits<float>::min()如果代表的是float的负数中的最小值确实没有什么意义。困扰我许久的问题终于解决了。
